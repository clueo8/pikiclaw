...
import type { Agent, StreamOpts } from '../agent/index.js';
...
Commands:
  skills          List project-defined custom skills (.pikiloom/skills)
  claude-run      Run a single Claude prompt and print the result
  codex-run       Run a single Codex prompt and print the result
  agy-run         Run a single Antigravity prompt and print the result
  gemini-run      Run a single Gemini prompt and print the result
  claude-status   Show Claude agent info and API usage
  codex-status    Show Codex agent info and API usage
  agy-status      Show Antigravity agent info and API usage
  ...
  agy-models      List available Antigravity models
  ...
  agy-sessions    List recent Antigravity sessions for the workdir
  ...
  agy-tail        Show last N messages of an Antigravity session
  ...
  switch (args.command) {
    case 'skills': {
      ...
      if (!result.skills.length) {
        process.stdout.write(`No custom skills found in ${workdir} (.pikiloom/skills)\n`);
        break;
      }
    ...
    case 'agy-status': case 'gemini-status': {
      const agent: Agent = args.command === 'gemini-status' ? 'gemini' : 'agy';
      const info = listAgents({ includeVersion: true }).agents.find(a => a.agent === agent)
        || listAgents({ includeVersion: true }).agents.find(a => a.agent === (agent === 'gemini' ? 'agy' : 'gemini'))!;
      const mark = info?.installed ? '\u2713' : '\u2717';
      process.stdout.write(`${mark} ${agent}  ${info?.version ?? 'not installed'}  ${info?.path ?? ''}\n`);
      const driver = getDriver(agent);
      const usage = driver.getUsageLive
        ? await driver.getUsageLive({ agent, model: args.model })
        : getUsage({ agent, model: args.model });
      if (usage.error) {
        process.stdout.write(`  ${usage.error}\n`);
      } else {
        for (const w of usage.windows) {
    ...
    case 'agy-models': case 'gemini-models': {
      const agent: Agent = args.command === 'gemini-models' ? 'gemini' : 'agy';
      const result = await listModels(agent, { workdir, currentModel: args.model });
      process.stdout.write(`${agent === 'agy' ? 'Antigravity' : 'Gemini'} models${result.note ? ` (${result.note})` : ''}:\n`);
      for (const m of result.models) {
        process.stdout.write(`  ${m.id}${m.alias ? ` (${m.alias})` : ''}\n`);
      }
      break;
    }
    case 'claude-sessions': case 'codex-sessions': case 'agy-sessions': case 'gemini-sessions': {
      const agent: Agent = args.command === 'codex-sessions'
        ? 'codex'
        : args.command === 'agy-sessions'
          ? 'agy'
    ...
    case 'claude-tail': case 'codex-tail': case 'agy-tail': case 'gemini-tail': {
      const agent: Agent = args.command === 'codex-tail'
        ? 'codex'
        : args.command === 'agy-tail'
          ? 'agy'
    ...
    case 'claude-run': case 'codex-run': case 'agy-run': case 'gemini-run': {
      const agent: Agent = args.command === 'codex-run'
        ? 'codex'
        : args.command === 'agy-run'
          ? 'agy'
          : args.command === 'gemini-run'
            ? 'gemini'
            : 'claude';
      ...
      const opts: StreamOpts = {
        ...
        agyModel: (agent === 'agy' || agent === 'gemini') ? (args.model || undefined) : undefined,
        geminiModel: (agent === 'agy' || agent === 'gemini') ? (args.model || undefined) : undefined,
      };
    ...
    default:
      ...
      process.stderr.write(`Available commands: skills, claude-run, codex-run, agy-run, gemini-run, claude-status, codex-status, agy-status, gemini-status, claude-models, codex-models, agy-models, gemini-models, claude-sessions, codex-sessions, agy-sessions, gemini-sessions, claude-tail, codex-tail, agy-tail, gemini-tail\n`);
      process.exit(1);
  }
}
